...
void WINAPI ServiceMain(DWORD argc, LPTSTR *argv) 
{
    ...
    wcscat_s(LogPath, sizeof(LogPath), L"\\logs\\MemoryMappedFileTest.log");
...
void FileMappingTest(void)
{
    ...
    {
        logs("CreateFileMapping function fail.");
        return;
    }
    else
    {
        LPTSTR lpMapAddr = (LPTSTR)MapViewOfFile(FileMappingHandle, FILE_MAP_ALL_ACCESS,0, 0, 0);
        memset(lpMapAddr, 0, MemSize);
        UnmapViewOfFile(lpMapAddr);
        logs("CreateFileMapping function success. Clear data on mappingFile.");
    }    
}
